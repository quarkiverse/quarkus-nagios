...
A health check that will always return OK, after it has returned OK once. This is useful if an application has multiple start-up checks, where each is expected to pass once the application is initialized.

==== UniSoftCache

A soft cache that will re-evaluate the supplier after a configurable amount of time. This is useful to avoid overloading the underlying system with health checks.
